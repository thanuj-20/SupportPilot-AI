...
POST /api/workflow/run        — run full multi-agent pipeline
GET  /api/workflow/history    — paginated workflow run history
GET  /api/workflow/{id}       — single workflow result
GET  /api/jira/tickets        — paginated Jira records
GET  /api/jira/stats          — Jira summary stats
GET  /api/emails              — paginated email records
GET  /api/emails/stats        — email summary stats
GET  /api/integrations/status — combined integration health
"""
import re
import logging
import uuid
...
async def run_workflow_endpoint(req: WorkflowRequest):
    ...
    try:
        ...
        email = await create_email(
            ticket_id, req.subject, diagnosis, resolution, escalation,
            user_email=req.user_email,
        )

        # Save escalation record if escalated (upsert — idempotent)
        await save_escalation(
            ticket_id, req.subject, diagnosis, resolution, escalation,
            workflow_id=result["workflow_id"],
        )

        # Save ticket resolution record (upsert by ticket_id)
        db = get_db()
        ...
        # Persist workflow run (upsert by workflow_id — prevents duplicates on retry)
        run_doc = {
            **result,
            "jira_id":    jira["jira_id"],
            "email_sent": True,
        }
        if "retrieval" in run_doc and "chunks" in run_doc.get("retrieval", {}):
            run_doc["retrieval"] = {
                k: v for k, v in run_doc["retrieval"].items() if k != "chunks"
            }
...
# ── Workflow Monitoring (MUST be before /{workflow_id}) ──────────────────────
